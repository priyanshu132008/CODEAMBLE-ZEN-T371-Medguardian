...
async def _generate(system: str, user: str) -> Tuple[str, str]:
    """Call the local Mistral proxy and return (body, body_source).

    ``body_source`` is ``"llm"`` when the email was genuinely generated by the
    model, or ``"template_fallback"`` when the LLM was unavailable / returned
    empty and a plainly templated body was used instead. The caller surfaces
    this so a canned fallback is never delivered as if it were an AI-written
    email — the recipient/UI can always tell the difference.
    ...
    try:
        ...
        if body:
            return body, "llm"
        return _fallback_body(user), "template_fallback"
...
async def dispatch_care_coordination(
    ...
    # Each _generate call returns (body, body_source) so we can label whether
    # each email was AI-written or a templated fallback (never silently faked).
    (doctor_body, doctor_source), (patient_body, patient_source) = await asyncio.gather(
        _generate(
    ...
    return {
        ...
        "detail": detail,
        "language": language,
        # Honest labeling: "llm" = model-generated body, "template_fallback" =
        # canned body used because the LLM was unavailable. Surfaces the
        # degradation so the canned email is never mistaken for AI output.
        "body_source": {
            "doctor_email": doctor_source,
            "patient_email": patient_source,
        },
        "doctor_email": {
            "to": doctor_email,
